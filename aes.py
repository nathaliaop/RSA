...
class AES:
    def __init__(self, key):
        ...
        for i in range(4, 4 * 11):
            ...
            if i % 4 == 0:
                byte = self.round_keys[i - 4][0] ^ SUBSTITUTION_BOX[self.round_keys[i - 1][1]] ^ R_TABLE[i // 4]
                self.round_keys[i].append(byte)

                for j in range(1, 4):
                    byte = self.round_keys[i - 4][j] ^ SUBSTITUTION_BOX[self.round_keys[i - 1][(j + 1) % 4]]
                    self.round_keys[i].append(byte)
            else:
                for j in range(4):
    ...
    def __inv_sub_bytes(self, s):
        for i in range(4):
            for j in range(4):
                s[i][j] = SUBSTITUTION_BOX_INVERTED[s[i][j]]
    ...
    def __inv_mix_columns(self, s):
        ...
        for i in range(4):
            u = self.__multiply(self.__multiply(s[i][0] ^ s[i][2]))
            v = self.__multiply(self.__multiply(s[i][1] ^ s[i][3]))
            s[i][0] ^= u
            s[i][1] ^= v
            s[i][2] ^= u
            s[i][3] ^= v
